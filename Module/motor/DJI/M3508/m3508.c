...
M3508_Status_t m3508_init(FDCAN_HandleTypeDef *hcan, const uint32_t txid, const uint8_t num)
{
    if (num > M3508_MAXNUM)
...
M3508_Status_t m3508_ctrl(const m3508_instance* m3508_ins, int16_t current[4])
{
    ...
    if (BSP_CAN_Transmit(can_ins, m3508_ins->txid, FDCAN_STANDARD_ID, txdata, 8) != CAN_OK)
        return M3508_ERROR;
