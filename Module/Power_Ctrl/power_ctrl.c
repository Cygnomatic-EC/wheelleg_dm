...
static PowerControllerConfig config = {0};
static PowerAllocationResult result = {0};
 ...
 * @brief 初始化功率控制器配置
 */
void initPowerControllerConfig(const float torqueConst, const float currentLimit, const float outputLimit,
 ...
 * @brief 设置最大功率
 */
void setMaxPower(const float maxPower) // 裁判系统功率限制+超电
{
    config.maxPower = maxPower;
}
 ...
 */
uint8_t limitMaxPower(const float buffer)
{
