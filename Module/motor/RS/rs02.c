...
RS02_Status_t rs02_init(FDCAN_HandleTypeDef *hcan, const uint8_t motorid, const uint8_t masterid, const uint8_t mode, const uint8_t protocol)
{
    uint8_t index = 0;
...
void rs02_change_masterid_mit(rs02_instance* rs02_ins, const uint8_t masterid)
{
    ...
    tx_data[6] = masterid;
    BSP_CAN_Transmit(rs02_ins->can_ins, rs02_ins->motorid, FDCAN_STANDARD_ID, tx_data, 8);
}
...
void rs02_enable_private(const rs02_instance* rs02_ins)
{
    ...
    BSP_CAN_Transmit(rs02_ins->can_ins, txid, FDCAN_EXTENDED_ID, tx_data, 8);
}
